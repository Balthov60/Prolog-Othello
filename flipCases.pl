...
flipPion(Plateau, X, Ymax, Ymax, Plateau).

flipPion(Plateau, X, Y, Ymax, R):-
     case(X, Y, Plateau, Z),
     (Z = 'n' -> replace(X, Y, 'b', Plateau, R2);
     Z = 'b' -> replace(X, Y, 'n', Plateau, R2);
     fail),
        Y2 is Y + 1,
        flipPion(R2, X, Y2, Ymax, R).        
     
flipPionsSurLigne(Plateau,Dir, Borne1, Borne2, R) :- 
    ((Borne2>Borne1) -> Borne_inf is Borne1+1, Borne_sup is Borne2, flipPionsVersDir(Plateau,Dir, Borne_inf, Borne_sup, R);
     Borne_inf is Borne2+1, Borne_sup is Borne1-1,flipPionsVersDir(Plateau,Dir, Borne_inf, Borne_sup, R)).
     
flipPionsVersDir(Plateau,Dir, Borne_inf, Borne_sup, R) :-
    	flipPion(Plateau,Dir, Borne_inf, Borne_sup, R).
